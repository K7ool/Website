...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("[LICENSE_VERIFY] Fields validated, querying Firestore");
    ...
    console.log("[LICENSE_VERIFY] License found in Firestore");
    ...
    console.log("[LICENSE_VERIFY] Fetching product version");
...
export async function GET(req: NextRequest) {
  ...
  if (productId) {
    ...
    try {
      ...
      if (prod) productName = prod.name || prod.productName || "Unknown Product";
      const versionsSnap = await adminDb
        ...
        .limit(1)
        .get();
      if (!versionsSnap.empty) latestVersion = versionsSnap.docs[0].data().version || "1.0.0";
    } catch {
      // fallback
    }
