...
export async function POST(req: NextRequest) {
  try {
    ...
    if (licSnap.empty) {
      return NextResponse.json({ success: true, revoked: false, active: false, reason: "LICENSE_NOT_FOUND" });
    }

    const lic = licSnap.docs[0].data();

    if (lic.status === "revoked") {
      return NextResponse.json({ success: true, revoked: true, active: false, reason: "LICENSE_REVOKED" });
    }

    if (lic.expiresAt && new Date(lic.expiresAt) < new Date()) {
      await adminDb.collection("licenses").doc(licSnap.docs[0].id).update({ status: "expired" });
      return NextResponse.json({ success: true, revoked: false, active: false, reason: "LICENSE_EXPIRED" });
    }

    if (lic.status !== "active") {
      return NextResponse.json({ success: true, revoked: false, active: false, reason: `LICENSE_${lic.status?.toUpperCase() || "INACTIVE"}` });
    }

    return NextResponse.json({ success: true, revoked: false, active: true });
  } catch (err: any) {
